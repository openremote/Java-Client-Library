...
/**
 * Base class for widgets that are capable of connecting to sensors. Sensors are
 * connected to various properties on the widgets and changes to the sensor will
 * result in property change events on the connected property. TODO: Sensor link
 * should provide information about which property it is linked to.
 * 
...
public abstract class SensoryWidget extends Widget {
  ...
  /**
   * Called by the controller service whenever a linked sensor's value changes.
   * The panel that owns this widget must be registered with the controller
